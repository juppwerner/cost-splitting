...
class ExcelGrid extends \yii\grid\GridView
{
	...
	public function run(){
        //$this->test();
        Yii::info('init_provider', 'ExcelGrid');
		$this->init_provider();
        Yii::info('init_excel_sheet', 'ExcelGrid');
		$this->init_excel_sheet();
        Yii::info('initPHPExcelWriter', 'ExcelGrid');
		$this->initPHPExcelWriter('Excel2007');
        Yii::info('generateHeader', 'ExcelGrid');
		$this->generateHeader();
        Yii::info('generateBody', 'ExcelGrid');
        $row = $this->generateBody();
        Yii::info('generateFiltersSheet', 'ExcelGrid');
        $this->generateFiltersSheet();
	...
	public function init_excel_sheet(){
		...
		$title = 'Applications';
		$subject = '';
		$description = 'Excel Grid Generated By PfeA application';
		$category = '';
		$keywords = '';
		$manager = '';
		$company = 'Stellantis';
    ...
    public function generateFiltersSheet()
    {
        $this->_objPHPExcel->createSheet();
        $this->_objPHPExcel->setActiveSheetIndex(1);
        $this->_objPHPExcel->getActiveSheet()->setTitle('Filters');
        $this->_objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setSize(14);
        $this->_objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setBold(true);
        
        $this->_objPHPExcel->getActiveSheet()->setCellValue('A1', 'Filters');
        $classname = basename(get_class($this->filterModel));
        $start = 3;
        foreach($this->_provider->sort->params[$classname] as $attribute=>$value) {
            if($value==='')
                continue;
            $this->_objPHPExcel->getActiveSheet()->setCellValue('A'.$start, $attribute.':');
            $this->_objPHPExcel->getActiveSheet()->setCellValue('B'.$start, $value);
            $start++;
        }
        $this->_objPHPExcel->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
        $this->_objPHPExcel->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);
